...
{
	WCHAR const* pFilePathConverted;
#ifndef UNICODE
		size_t size = strlen(pFilePath) + 1;
		std::wstring wideString(size, L'\0');
		mbstowcs(wideString.data(), pFilePath, size);
		pFilePathConverted = wideString.c_str();
#else
	pFilePathConverted = pFilePath;
#endif

	Gdiplus::Bitmap* bitmapPtr = Gdiplus::Bitmap::FromFile(pFilePathConverted, false);
